...
public:
    ...
    T deriv(u32 index, const T& grad, const T& arg1, const T& arg2) override {
        if (index == 0) {
            return mmul(grad, transpose(arg2));
        } else {
            return mmul(transpose(arg1), grad);
        }
    }
    ...
    std::shared_ptr<Op<T>> derivFunc(u32 index, std::shared_ptr<Op<T>> grad, std::shared_ptr<Op<T>> arg1, std::shared_ptr<Op<T>> arg2) override {
        if (index == 0) {
            return std::make_shared<Mmul<T>>(grad, std::make_shared<Transpose<T>>(arg2));
        } else {
            return std::make_shared<Mmul<T>>(std::make_shared<Transpose<T>>(arg1), grad);
        }
    }
