...
public:
    ...
    GradDescent(std::shared_ptr<op::Op<base::Tensor<f64>>> cost_op, 
        ...
        f64 init_step, f64 rho, f64 tangent_slope_coef, u32 max_iter_cnt, u32 max_probe_retries): 
        _cost_op(cost_op), _var_vec(var_vec), _init_step(init_step), _rho(rho), _tangent_slope_coef(tangent_slope_coef), _max_iter_cnt(max_iter_cnt), _max_probe_retries(max_probe_retries) {   
        if (_rho <= 0 || _rho >= 1) {
...
private:
    ...
    u32 _max_iter_cnt, _max_probe_retries;
};
