...
public:
    ...
    const T& getOutput() const {
        if (!_has_output) {
            LOG(ERROR) << "[" << __FUNCTION__ << "]" << "touch unset output: " << google::GetStackTrace();
    ...
    const T& getGrad() const {
        if (!_has_grad) {
            LOG(ERROR) << "[" << __FUNCTION__ << "]" << "touch unset grad: " << google::GetStackTrace();
    ...
    std::shared_ptr<Op<T>> getGradGraph() const {
        if (!_grad_graph) {
            LOG(ERROR) << "[" << __FUNCTION__ << "]" << "touch null grad graph: " << google::GetStackTrace();
